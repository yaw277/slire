...
For applications where consistency and convenience are priorities, a single comprehensive factory provides all data access capabilities through one interface (simplified, partially fictionalized example):
...
**Why no session in context?**

Since this unified approach provides client-side stored procedures that manage their own transactions internally, including a session parameter in the context would create inconsistent behavior. Some operations would use the context session while others would manage their own sessions, leading to confusion about when and how transactions are being used.

Instead, each operation that needs transactional behavior handles session management internally, keeping the context simple and the transactional boundaries clear.
...
export function createDataAccess(context: DataAccessContext): DataAccess {
  const { organizationId, mongoClient, logger } = context;
  ...
  return {
    expenses: {
      ...
      receipts: {
        insertReceiptExpenseAndStartTracking:
          createInsertReceiptExpenseAndStartTracking({
            expenseRepo,
            logger,
          }),
        updateReceiptFromDigitizeCallback:
          createUpdateReceiptFromDigitizeCallback({
            expenseRepo,
            logger,
          }),
        updateReceiptFromConversionCallback:
          createUpdateReceiptFromConversionCallback({
            expenseRepo,
            logger,
          }),
      },
      ...
      perDiems: {
        getRelevantHistoricPerDiemExpenses:
          createGetRelevantHistoricPerDiemExpenses(expenseRepo),
        removeTaxabilityFromPerDiemExpenses:
          createRemoveTaxabilityFromPerDiemExpenses({
            expenseRepo,
            logger,
          }),
        getPerDiemExpensesToBeRecomputed:
          createGetPerDiemExpensesToBeRecomputed(expenseRepo),
      },
    },
    ...
    trips: {
      ...
      deleteAutogeneratedTripExpenses: createDeleteAutogeneratedTripExpenses({
        tripRepo,
        expenseRepo,
        logger,
      }),
      updateLinkedTrips: createUpdateLinkedTrips({
        tripRepo,
        expenseRepo,
        logger,
      }),
    ...
    users: {
      repo: userRepo,
      getRecentUserPreferences: createGetRecentUserPreferences(
        userRepo,
        expenseRepo
      ),
    ...
    composite: {
      importExpensesFromCsv: createImportExpensesFromCsv({
        expenseRepo,
        tripRepo,
        userRepo,
        logger,
      }),
      validateDataConsistency: createValidateDataConsistency({
        expenseRepo,
        tripRepo,
        userRepo,
        logger,
      }),
      recalculateExpenseTotals: createRecalculateExpenseTotals({
        expenseRepo,
        tripRepo,
        logger,
      }),
      syncExpenseStatusAcrossTrips: createSyncExpenseStatusAcrossTrips({
        expenseRepo,
        tripRepo,
        logger,
      }),
